...
let x86_supp_instr =
  List.concat_map (fun (name, pc) -> pp_x86 name pc) X86_instr_decl.x86_prim_string
...
let pp_suff_arm name f a b : string list =
  let op = match f a b with
	| Ok o -> o
	| Error err -> failwith "ERROR: invalid suffix"
  in
...
let arm_supp_instr =
  List.concat_map (fun (name, pc) -> pp_arm name pc) Arm_instr_decl.arm_prim_string
  |> List.sort_uniq String.compare
